...
void adm_translate_source(String_View source, ADM *adm, Pasm *lt) {
    ...
    while (source.count > 0) {
        ...
        } else if (sv_eq(word, cstr_as_sv(inst_name(INST_NOP)))) {
            adm->program[adm->program_size++] = (Inst) {
                .type = INST_NOP
            };
        } else if (sv_eq(word, cstr_as_sv(inst_name(INST_PUSH)))) {
        ...
        } else if (sv_eq(word, cstr_as_sv(inst_name(INST_PLUSI)))) {
            adm->program[adm->program_size++] = (Inst){
                .type = INST_PLUSI
            };
        } else if(sv_eq(word, cstr_as_sv(inst_name(INST_HALT)))) {
            adm->program[adm->program_size++] = (Inst){
                .type = INST_HALT
            };
